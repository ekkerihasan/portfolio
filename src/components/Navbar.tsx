...
export default function Navbar() {
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      // If we are at the top (Hero), keep it visible and don't start timers.
      ...
      // 3. AUTO-HIDE LOGIC (Only runs if below 100px)
      ...
      scrollTimerRef.current = setTimeout(() => {
        // Only hide if the mobile menu isn't currently open
        setIsOpen((prevOpen) => {
    ...
    window.addEventListener("project-view-active", handleProjectView);
    ...
    return () => {
      ...
      window.removeEventListener("project-view-active", handleProjectView);
      if (scrollTimerRef.current) clearTimeout(scrollTimerRef.current);
    };
  }, [isModalOpen]);
